<div>
    <x-data-table-panitia :data="$data" :model="$pembayarans">
        <x-slot name="head">
            <tr>
            ...
            @foreach ($pembayarans as $key => $pembayaran)
                <tr x-data="window.__controller.dataTableController({{ $pembayaran->id }})">
                    ...
                    <td class="whitespace-no-wrap row-action--icon">
                        ...
                        <a role="button" target="_blank" href="{{ route('tu.show',$pembayaran->id) }}" class="mr-3"><i class="fas fa-print"></i></i></a>
                        <a role="button" x-on:click.prevent="deleteItem" href="#"><i class="text-red-500 fa fa-16px fa-trash"></i></a>
                    </td>
                </tr>
            @endforeach
        </x-slot>
    </x-data-table-panitia>
